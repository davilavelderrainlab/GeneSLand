...
This vignette is a quick guide to using the `GeneSLand` package. 
...
The framework consists in subdividing an expression matrix into bins of the same size from the lowest to the highest values of the matrix.
For each gene (or averaged gene group) and each bin the number of columns of the original expression matrix with an expression higher than the considered bin are counted. 
The expression bins correspond to the expression Level, while the number of columns with an expression higher than that bin is called expression Breadth.
...
The obtained SSCE now contains several informations: under `assays` there are, for each gene, the expression breadth (B) score for the 50 expression level (L) bins. Additionally, three
scores are provided: `AUC`, `dRate`, `lbSpec`, together with initial B scores. 
  - `AUC` corresponds to the area under the L-B curve and relates to the average expression of the gene;
  - `dRate` corresponds to how quickly the L-B curve drops to zero and relates to the expression variability of the gene;
  - `lbSpec` corresponds to how specific the expression of a gene is and is computed considering both in how many samples the gene is expressed and in how many samples the expression is
             much higher than the other samples.

The scores can be easily accessed by:
...
When the specificity scores are shown, a darker color is given to genes with lower values, whereas a brighter color corresponds to genes with higher values.
...
So, if for Gene-1 the line has a B value of 0.5 at L = 20, it means that 50% of the cell types express Gene-1 more than the expression value corresponding to the 20th bin. 
The bin values (like 20 in this case) are not counts, but obtained by dividing the expression range in the dataset (from maximum to minimum expression) in equally distant parts (50 by default). 
20 indicates the expression value that is 20/50 of the maximum expression value.

# Gene groups SSCE

We can also create SSCEs starting from gene sets of choice, like Gene Ontology Biological Processes, or HUGO gene groups. To do so, we just need to provide a list of gene sets, 
in which each element is a gene set and contains a series of genes.
...
The score significance indicates how statistically significant are the scores compared to a random gene sets with the same number of genes, 
whereas the variability shows how homogeneous the gene set is in terms of individual scores.
...
If we want to compare the expression behavior of genes in different contexts, an easy visualization can be obtained by mapping the LB behavior of genes onto an existing SSCE 
and see how they deviate. To do so, we just need to run the `mapSSCE` function giving a SSCE, the new expression profiles and the wanted gene or genes. 
Also in this case, we will generate the new profiles randomly.
